...
- #### interactions/potentials  
    ...
    ||sigma|float[]|LJパラメータ $\sigma$ ( $s^2$ 要素)( $[v_{00}, v_{01}, v_{10}, v_{11}]$ )|
    ||epsilon|float[]|LJパラメータ $\epsilon$ ( $s^2$ 要素)( $[v_{00}, v_{01}, v_{10}, v_{11}]$ )|
    ||cutoff|float[]|カットオフ距離 ( $s^2$ 要素)( $[v_{00}, v_{01}, v_{10}, v_{11}]$ )|
...
- #### observer (出力設定)
    ...
    ||is_unwrap|bool|PBCを展開するか否か|
    ||divisions|int| $10^1$ ステップ毎の分割数|
    |log_plus_stride_export_trajectory (ログ＋線形間隔保存)|output_path|string|出力先ディレクトリ (直下に{i}.xyz)で保存されます。|
    ||is_unwrap|bool|PBCを展開するか否か|
    ||divisions|int| $10^1$ ステップ毎の分割数|
