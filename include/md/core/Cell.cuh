...
namespace md{
    ...
    class Cell {
        public:
            Cell(int N, std::array<float, 3> _lattice);

            __forceinline__ __host__ __device__ void apply_pbc_device(float* x, float* y, float* z) const {
                *x -= lattice.x * roundf(*x * lattice_inv.x);
                *y -= lattice.y * roundf(*y * lattice_inv.y);
                *z -= lattice.z * roundf(*z * lattice_inv.z);
            }

            __forceinline__ __host__ __device__ void apply_pbc_wrap_device(float* x, float* y, float* z) const {
