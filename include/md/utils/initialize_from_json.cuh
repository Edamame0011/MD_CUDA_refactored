...
namespace md::utils::initialize {
    ...
    std::array<std::array<float, 3>, 3> init_state(md::State& state, const json& a_setting, std::mt19937& mt) {
        ...
        if (mode == "generate_binary_lj") {
            ...
            if (format == "xyz") {
                lattice = md::utils::initialize::read_state_from_xyz(state, a_setting.at("path"));
            } else {
    ...
    std::unique_ptr<md::Observer> build_observer(const json& o_setting) {
        ...
        if (o_type == "linear") {
            return std::make_unique<md::observers::LinearOutput>(o_setting.at("output_interval"));
        } else if (o_type == "log") {
    ...
    EnsembleComponents build_ensemble(const json& e_setting, md::State& state, std::mt19937& mt) {
        ...
        if (ensemble == "NVE") {
            ...
            if (thermo_type == "Nose-Hoover") {
                auto nhc = std::make_unique<md::thermostats::NoseHooverChain>(
                    e_setting.value("length", 1), e_setting.value("tau", 50), comp.scheduler.get());
                nhc->init(state);
    ...
    std::unique_ptr<md::Interaction> build_interaction(const json& p_setting, md::State& state, CellType& cell, md::utils::NeighbourList* nl) {
        ...
        if (p_type == "lennard_jones") {
            ...
            return std::make_unique<md::interactions::NNPTorchScript<CellType>>(state, cell, nl, p_setting.at("model_path"));
        }
