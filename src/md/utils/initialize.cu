...
namespace md::utils {
    ...
    std::unique_ptr<md::State> read_state_from_xyz(std::unique_ptr<Cell>& cell, const std::string& path) {
        ...
        cell = std::make_unique<Cell>(N, lattice);

        // 原子番号の種類を取得
        std::vector<int> unique_atomic_numbers = h_atomic_numbers;

        // 昇順にソート
        std::sort(unique_atomic_numbers.begin(), unique_atomic_numbers.end());

        // 重複を削除
        unique_atomic_numbers.erase(
            std::unique(unique_atomic_numbers.begin(), unique_atomic_numbers.end()),
            unique_atomic_numbers.end()
        );

        // atomic number -> species ID に変換
        for (int i = 0; i < N; ++i) {
            auto it = std::lower_bound(
                unique_atomic_numbers.begin(),
                unique_atomic_numbers.end(),
                h_atomic_numbers[i]
            );
        
            h_species[i] = (int)std::distance(unique_atomic_numbers.begin(), it);
        }
        ...
        state->init_normal(
            ...
            h_species.data(), h_masses.data(), h_atomic_numbers.data()
        );
