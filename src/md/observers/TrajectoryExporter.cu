...
TrajectoryExporter::TrajectoryExporter(State& state, const std::string& output_path) 
...
void TrajectoryExporter::export_trajectory(State& state) {
    ...
    ofs << N << "\n";
    ofs << "Properties=species:S:1:pos:R:3:forces:R:3 energy=" << state.potential_energy << " pbc=T T T" << "\n";
    for (size_t i = 0; i < N; i ++) {
        ofs << species[i] << " "
