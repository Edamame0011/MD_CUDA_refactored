...
void NNP<CellType>::calc_force(State& state) {
    ...
    edge_weight = torch::from_blob(edge_weight_ptr, {3, num_edges}, opt.dtype(torch::kFloat32));
...
void NNP<CellType>::calc_potential(State& state) {
    ...
    x = torch::from_blob(x_ptr, {N}, opt.dtype(torch::kInt64));
    edge_index = torch::from_blob(edge_index_ptr, {2, num_edges}, opt.dtype(torch::kInt64)).contiguous();
    edge_weight = torch::from_blob(edge_weight_ptr, {3, num_edges}, opt.dtype(torch::kFloat32)).contiguous();
