#include <md/utils/initialize_from_json.cuh>

#include <external/nlohmann/json.hpp>
#include <fstream>
#include <string>
#include <chrono>
#include <cmath>

using json = nlohmann::json;

template <typename CellType>
void simulate(CellType& cell, const json& j) {
    json m_setting = j.at("meta");
    json a_setting = j.at("atoms");
    json s_setting = j.at("simulation");
    json o_setting = j.at("output");
    json i_setting = j.at("interactions");

    md::State state;
    
    // その他の設定
    int rand_seed = m_setting.value("seed", 12345);
    std::mt19937 mt(rand_seed);
    state.dt = s_setting.at("dt");
    state.current_steps = 0;
    
    // ユニットの初期化
    md::utils::initialize::configure_units(m_setting);

    // 系の初期化
    auto lattice = md::utils::initialize::init_state(state, a_setting, mt); 
    cell.init(lattice);

    // 隣接リストとポテンシャルの初期化
    json nl_setting = i_setting["neighbour_list"];
    float cutoff = nl_setting.value("cutoff", 5.0f);
    float margin = nl_setting.value("margin", 1.0f);
    md::utils::NeighbourList nl(cutoff, margin);
    nl.generate(state, cell);
    auto interaction = md::utils::initialize::build_interaction(i_setting.at("potentials"), state, cell, &nl);

    // アンサンブルの初期化
    auto ensemble = md::utils::initialize::build_ensemble(s_setting.at("ensemble"), state, mt);

    // オブザーバーの初期化
    auto observer = md::utils::initialize::build_observer(o_setting);

    // simulatorの初期化
    md::Simulator simulator(state, interaction.get(), ensemble.integrator.get(), observer.get(), cell);

    // 時間の計測
    auto start = std::chrono::steady_clock::now();

    // シミュレーションの実行
    simulator.run(s_setting.at("simulation_time"));
    
    auto end = std::chrono::steady_clock::now();
    double elapsed_s = std::chrono::duration<double>(end - start).count();

    std::cout << "かかった時間：" << elapsed_s << "s" << std::endl;
}

int main(int argc, char* argv[]) {
    try {
        // コマンドライン引数の処理
        if (argc < 2) {
            std::cerr << "jsonファイルのパスを入力してください。" << std::endl;
            return 1;
        }
    
        std::string json_path = argv[1];
    
        // jsonファイルの処理
        std::ifstream f(json_path);
        if (!f.is_open()) throw std::runtime_error("ファイルを開けません。" );
        json j = json::parse(f);
    
        json s_setting = j.at("simulation");
    
        std::string cell_type = s_setting.value("cell_type", "cubic");
        if (cell_type == "cubic") {
            md::cells::CubicCell cell;
            simulate(cell, j);
        }
        else {
            throw std::runtime_error("未対応のcell typeです: " + cell_type);
        }
    }
    catch (const std::exception& e) {
        std::cerr << "[Error]" << e.what() << std::endl;
        return 1;
    }
    return 0;
}